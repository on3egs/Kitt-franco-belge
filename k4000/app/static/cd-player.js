(() => {
  ...
  function setVolume(value) { state.volume = Math.max(0, Math.min(100, Number(value))); volume.value = state.volume; volume.style.setProperty('--volume-level', `${state.volume}%`); $('cdVolumeValue').textContent = `${state.volume}%`; applyVolume(); persist(); }
