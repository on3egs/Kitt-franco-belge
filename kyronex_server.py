...
_cam_proc = None  # subprocess cam_stream.py
...
def _start_cam_process():
    ...
    if _cam_proc is None or _cam_proc.poll() is not None:
        _cam_proc = _sp.Popen(
            ["/usr/bin/python3", _CAM_STREAM_SCRIPT],
            stdout=_sp.DEVNULL, stderr=_sp.DEVNULL
        )
        print(f'[CAM] Process demarre pid={_cam_proc.pid}', flush=True)
...
async def handle_camera_stream(request: web.Request) -> web.StreamResponse:
    ...
    try:
        while CAMERA_STREAM_ENABLED:
            try:
                with open(_CAM_FRAME_FILE, 'rb') as _f:
                    frame = _f.read()
...
def create_app() -> web.Application:
    ...
    async def stop_background(app):
        for key in ("stats_task", "cleanup_task", "proactive_task", "reminder_task", "llm_warmup_task"):
            task = app.get(key)
            if task:
                task.cancel()
