...
interface PdfEntry {
  id: string; url: string; titre: string; description: string;
  categorie: string; pseudo: string; ts: number; views?: number;
}
...
export default function AdminPdfs() {
  ...
  const [pending, setPending] = useState<PdfEntry[]>([]);
  const [approved, setApproved] = useState<PdfEntry[]>([]);
  ...
  useEffect(() => {
    ...
    getApiBase().then(base => {
      ...
      fetch(`${base}/api/pdfs/pending`, { headers: { "X-Admin-Token": ADMIN_TOKEN } })
        .then(r => r.json())
  ...
  async function decide(id: string, action: "approve" | "reject") {
    ...
    setDeciding(id);
    try {
      await fetch(`${apiBase}/api/pdfs/decide`, {
        method: "POST",
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: "#0a0000" }}>
        <div className="w-full max-w-sm p-8" style={{ background: "#1a0000", border: "2px solid #ff2222", boxShadow: "0 0 30px rgba(255,34,34,0.2)" }}>
          ...
          <h1 className="text-2xl font-black mb-8" style={{ fontFamily: "Orbitron, monospace", color: "#ffffff" }}>MODÉRATION PDF</h1>
  ...
  return (
    <div className="min-h-screen" style={{ background: "#0a0000" }}>
      <div className="container py-16 max-w-3xl mx-auto px-4">
        <div className="mb-8">
          <div style={{ ...label, color: "rgba(255,34,34,0.5)", marginBottom: "8px" }}>// PANNEAU ADMIN — MODÉRATION PDF</div>
          <h1 className="text-3xl font-black text-white" style={{ fontFamily: "Orbitron, monospace" }}>DOCUMENTS SOUMIS</h1>
        </div>
        ...
        {!loading && apiBase && (
          <>
            <div style={{ ...label, color: "rgba(255,34,34,0.6)", marginBottom: "12px", marginTop: "24px" }}>
              // EN ATTENTE — {pending.length} document{pending.length !== 1 ? "s" : ""}
            </div>
            ...
            {pending.map(p => (
              <div key={p.id} style={cardStyle}>
                <div style={{ fontFamily: "Orbitron, monospace", fontSize: "0.6rem", color: "#fff", marginBottom: "2px" }}>{p.titre}</div>
                ...
                {p.description && (
                  <p style={{ fontFamily: "Rajdhani, sans-serif", fontSize: "0.9rem", color: "rgba(192,192,192,0.65)", fontStyle: "italic", marginBottom: "8px" }}>"{p.description}"</p>
                )}
                <div className="flex gap-3 mt-3">
                  <button onClick={() => decide(p.id, "approve")} disabled={deciding === p.id}
                  ...
                  <button onClick={() => decide(p.id, "reject")} disabled={deciding === p.id}
            ...
            <div style={{ ...label, color: "rgba(0,200,80,0.5)", marginBottom: "12px", marginTop: "32px" }}>
              // APPROUVÉS — {approved.length} document{approved.length !== 1 ? "s" : ""}
            </div>
            {approved.length === 0 && <p style={{ fontFamily: "Rajdhani, sans-serif", color: "rgba(192,192,192,0.4)" }}>Aucun document approuvé.</p>}
            {approved.map(p => (
              <div key={p.id} style={{ ...cardStyle, borderColor: "rgba(0,200,80,0.15)" }}>
                <div style={{ fontFamily: "Orbitron, monospace", fontSize: "0.55rem", color: "#fff", marginBottom: "2px" }}>{p.titre}</div>
                <div style={{ fontFamily: "Space Mono, monospace", fontSize: "0.45rem", color: "rgba(255,100,0,0.7)", marginBottom: "4px" }}>{p.categorie}</div>
                <div style={{ fontFamily: "Space Mono, monospace", fontSize: "0.48rem", color: "rgba(192,192,192,0.4)" }}>
                  {p.pseudo} — {formatDate(p.ts)} {p.views ? `— 👁 ${p.views}` : ""}
                </div>
              </div>
            ))}
          </>
